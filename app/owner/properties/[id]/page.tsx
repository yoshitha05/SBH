...
//
// Rebuilt from scratch with full Supabase integration AND owner_id
// scoping built in from the start. Covers: property info (load/edit),
// tenants in this building, Add Tenant modal, Expenditure (real table),
// Notes (real table, scoped to this building + this owner).
...
type ExpRow = { id: number; date: string; item: string; amount: number };
type NoteEntry = { id: number; content: string; created_at: string };

const EMPTY_NEW_TENANT_FORM = { name: "", flatNo: "", rent: 0, phone: "", email: "" };
...
export default function OwnerBuildingPage() {
  ...
  async function loadAll() {
    ...
    if (!propData) {
      setLoading(false);
      return; // property stays null -> "not found" state below
    }

    setProperty(propData);
    setAddress(propData.address);
    setTotalFlats(propData.total_flats);

    const { data: tenantData } = await supabase
      .from("tenants")
      .select("*")
      .eq("owner_id", user.id)
      .eq("building", propData.name)
      .order("flat_no", { ascending: true });

    setBuildingTenants(tenantData ?? []);
    setLoading(false);
  }
  ...
  async function addExpRow() {
    ...
    const { data, error } = await supabase
      .from("expenditure")
      .insert({ owner_id: property.owner_id, property_id: property.id, date: today, item: "", amount: 0 })
      .select()
      .single();

    if (error) { setExpError(error.message); return; }
    setExpRows((prev) => [data, ...prev]);
  }
  async function updateExpRow(rid: number, field: keyof ExpRow, val: string | number) {
    setExpRows((prev) => prev.map((r) => r.id === rid ? { ...r, [field]: val } : r));
    const { error } = await supabase.from("expenditure").update({ [field]: val }).eq("id", rid);
    if (error) setExpError(error.message);
  }
  async function deleteExpRow(rid: number) {
    if (!confirm("Delete this expense entry?")) return;
    const { error } = await supabase.from("expenditure").delete().eq("id", rid);
    if (error) { setExpError(error.message); return; }
  ...
  function downloadExcel() {
    if (!property) return;
    const rows = expRows.map((r) => ({ Date: r.date, Item: r.item || "(untitled)", "Amount (₹)": r.amount }));
    ...
    XLSX.writeFile(wb, `${property.name}_Expenditure_${new Date().toISOString().split("T")[0]}.xlsx`);
  ...
  function downloadPDF() {
    ...
    doc.save(`${property.name}_Expenditure_${new Date().toISOString().split("T")[0]}.pdf`);
    setShowExportMenu(false);
  }
  ...
  async function addNote() {
    ...
    const { data, error } = await supabase
      .from("notes")
      .insert({ owner_id: property.owner_id, property_id: property.id, content: newNoteText.trim() })
      .select()
      .single();
  ...
  async function saveEditNote() {
    ...
    setNotesError("");
    const { error } = await supabase.from("notes").update({ content: editNoteText.trim() }).eq("id", editingNoteId);
    if (error) { setNotesError(error.message); return; }
    setNotesList((prev) => prev.map((n) => n.id === editingNoteId ? { ...n, content: editNoteText.trim() } : n));
    setEditingNoteId(null);
    setEditNoteText("");
  }
  ...
  if (loadError) {
    return (
      <div className="p-10 text-center" style={{ color: "#A32D2D" }}>
        ...
        <p className="text-sm font-medium">Couldn't load this building</p>
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {tab === "overview" && (
        <div className="rounded-xl overflow-hidden mb-5" style={{ border: "1.5px solid rgba(27,79,187,0.18)" }}>
          ...
          <div className="grid text-xs font-semibold uppercase tracking-wide px-4 py-2"
            style={{ gridTemplateColumns: "1.5fr 0.6fr 1fr 0.8fr 0.8fr", background: "#FAFBFF", color: "#6B7280", borderBottom: "1px solid rgba(27,79,187,0.08)" }}>
            <div>Tenant</div><div>Flat</div><div>Rent</div><div>Approval</div><div>Access</div>
          </div>
          ...
          {buildingTenants.length > 0 ? (
            buildingTenants.map((tenant, i) => (
              <div key={tenant.id} className="grid px-4 py-3 items-center text-sm hover:bg-blue-50 transition"
                style={{ gridTemplateColumns: "1.5fr 0.6fr 1fr 0.8fr 0.8fr", borderTop: i===0?"none":"1px solid rgba(27,79,187,0.07)", background: "#fff" }}>
                <div>
      ...
      {tab === "history" && (
        <div className="rounded-xl p-8 text-center" style={{ border: "1.5px solid rgba(27,79,187,0.18)", color: "#9CA3AF" }}>
          <Clock size={24} className="mx-auto mb-2 opacity-30" />
          <p className="text-sm">Flat history isn't connected to real data yet — coming in a future update.</p>
        </div>
      )}
        ...
        <div>
          <div className="rounded-xl p-4 mb-4" style={{ border: "1.5px solid rgba(27,79,187,0.18)", background: "#F5F7FB" }}>
            ...
            <button onClick={addNote} disabled={!newNoteText.trim()}
              ...
              style={{ background: newNoteText.trim() ? "#1B4FBB" : "#D1D5DB", color: "#fff", border: "none", cursor: newNoteText.trim() ? "pointer" : "not-allowed" }}>
                  ...
                  {editingNoteId === note.id ? (
                    <>
                      <textarea value={editNoteText} onChange={(e) => setEditNoteText(e.target.value)}
                        className="w-full px-3 py-2 text-sm rounded-lg resize-y min-h-20"
                        style={{ border: "1.5px solid #1B4FBB", outline: "none", color: "#111827", lineHeight: 1.6 }} autoFocus />
                      <div className="flex gap-2 mt-2">
                        <button onClick={saveEditNote} disabled={!editNoteText.trim()}
                          className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium"
                          style={{ background: editNoteText.trim() ? "#1B4FBB" : "#D1D5DB", color: "#fff", border: "none", cursor: editNoteText.trim() ? "pointer" : "not-allowed" }}>
                          <Save size={11} /> Save
                        </button>
