...
//
// Converted to Supabase: sheets and their custom columns live in
// ledger_sheets (columns stored as jsonb), and each sheet's rows live in
// ledger_rows (each row's data stored as jsonb, keyed by sheet_id).
// This keeps the fully custom/user-defined column shapes from the
// original local-state version while making everything real and
// persistent.
//
// Notes tab stays local for this stage — a smaller follow-up can wire
// that to the same `notes` table we built for buildings, scoped
// differently (e.g. a "general" pseudo-property or its own table).
...
const CATS = ["Maintenance", "Utilities", "Staff", "Repairs", "Legal", "Other"];
...
export default function AdminExpenditurePage() {
  ...
  // Notes now save to the real `notes` table — "general" notes have a
  // Notes now save to a separate, dedicated `expenditure_notes` table —
  // fully independent from the `notes` table used by each building's own
  // detail-page Notes tab, so the two never collide. `scope` is just the
  // plain tab label ("general", "Ohm", etc.) stored as text.
  ...
  // Load the note for whichever tab ("general" or a building name) is
  // currently selected, from the dedicated expenditure_notes table.
  useEffect(() => {
  ...
  async function deleteSheet(id: number) {
    ...
    // Delete rows first (foreign key), then the sheet itself.
  ...
  // ── Keyboard nav within active sheet's grid ──
  ...
  // ── Export (operates on the currently active sheet) ──
  ...
  if (!activeSheet) {
    return (
      <div className="p-10 text-center" style={{ color: "#9CA3AF" }}>
        ...
        <button onClick={openNewSheetModal} className="text-xs mt-2" style={{ color: "#1B4FBB" }}>+ Create your first sheet</button>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {/* Metric cards — only meaningful for the default ledger sheet */}
      {mainTab === "ledger" && isDefaultLedger && (
      ...
      {/* Main tabs (Ledger / MS Excel / Notes / Export) */}
      ...
      {mainTab === "ledger" && (
        <div>
          ...
          {/* Sheet tabs row, Excel-style, with + to add */}
      ...
      {/* ── NOTES TAB — still local for this stage (see note at top of file) ── */}
      {mainTab === "notes" && (
        <div>
