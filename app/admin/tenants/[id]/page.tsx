...
export default async function AdminTenantDetailsPage({
  ...
  const tenant = tenants.find((t) => t.id === id) as (typeof tenants[number] & {
    age?: number;
    aadhar?: string;
    coTenants?: { id: string; name: string; age: string; phone: string; email: string; aadhar: string }[];
  }) | undefined;
  if (!tenant) notFound();
  ...
  const occupants = 1 + (tenant.coTenants?.length ?? 0);
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm mb-4" style={{ color: "#6B7280" }}>
        <Link href={`/admin/tenants/${tenants.id}`} className="text-xs font-semibold hover:underline" style={{ color: "#1B4FBB" }}>
      ...
      >
        <div className="flex items-start justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {tenant.approved && tenant.accessEnabled && (
              <Link
                href={`/admin/tenant/${tenant.id}?action=vacate`}
                className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-medium"
        ...
        {/* Co-tenants list, only shown if there are any */}
        {tenant.coTenants && tenant.coTenants.length > 0 && (
          <div className="mt-5 pt-5" style={{ borderTop: "1px solid rgba(27,79,187,0.1)" }}>
            <div className="flex items-center gap-1.5 text-xs font-semibold mb-3" style={{ color: "#6B7280" }}>
              <Users size={13} /> Other occupants in this flat ({tenant.coTenants.length})
            </div>
            <div className="grid md:grid-cols-2 gap-3">
              {tenant.coTenants.map((co) => (
